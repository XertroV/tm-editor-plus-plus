// AUTO-GENERATED from ml-scripts/EditorPlugin_EditorPlusPlus.Script.txt by pre-proc-scripts.py (run by build.sh).
// Do NOT edit this file — it is overwritten on every build; edit the ml-scripts source.
...
Void ProcessIncomingMessages() {
	...
	foreach (InMsg in EPP_MsgQueue) {
		...
		if (MsgType == "ResyncPlease") {
			...
			SendEvent("CustomColorTables", [""^CCT_CustomColorTables]);
		} else if (MsgType == "SetMapKV") {
			if (DisableMetadata || InMsg.count != 3) continue;
			if (TL::Length(InMsg[1]) <= 5) continue;
			if (TL::SubString(InMsg[1], 0, 5) != "_EKV_") continue;
			// Only create this dictionary on an explicit write. Values are whole
			...
			// 16384 is provisional and deliberately conservative: it is the
			// chunk size the inbound transport is already known to carry, and
			// nothing yet measures how large an argument LayerCustomEvent
			// delivers intact. A truncated echo would read as drift, so the
			// bound stays where a value is known to survive. To raise it,
			// measure first: write values of increasing length and confirm the
			// echoed TL::Length matches what was sent, then move this number
			// and the matching one in docs/MapKeyValues.md together.
			if (TL::Length(_EKV_[InMsg[1]]) <= 16384) {
				...
				// A length disagreement never fences the reader, because at this
				// size the echo itself is the unmeasured part.
