...
// size 0x340; was 0x330 before 2026.
// Inputs: Validate_GameModeCustomData + 0x18 = 0x1A8
// EntRecord: Validate_ExtraTool_Info + 0xC0 = 0x2F0
class DGameCtnGhost : RawBufferElem {
	DGameCtnGhost(RawBufferElem@ el) {
		if (el.ElSize != 0x340) throw("invalid size for DGameCtnGhost");
	...
	DGameCtnGhost(CGameCtnGhost@ nod) {
		...
		super(Dev_GetPointerForNod(nod), 0x340);
	}
	CGameCtnGhost@ get_Nod() {
		return cast<CGameCtnGhost>(Dev_GetNodFromPointer(ptr));
	}

	CPlugEntRecordData@ get_EntRecordData() { return cast<CPlugEntRecordData>(this.GetNod(O_CTN_GHOST_ENT_RECORD_DATA)); }
