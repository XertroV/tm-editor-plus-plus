...
namespace Gizmo {
    ...
    void GizmoLoop() {
        ...
        while (IsActive && (@editor = cast<CGameCtnEditorFree>(app.Editor)) !is null) {
            ...
            Editor::VehiclePreview::Follow(gizmo.GetCursorMat());
            yield();
        }
        IsActive = false;
    }
    ...
    void Gizmo_Setup(CGameCtnEditorFree@ editor) {
        ...
        if (S_Gizmo_ShowVehiclePreview) {
            if (shouldReplaceTarget && b !is null && Editor::VehiclePreview::HasSpawn(b.BlockInfo)) {
                Editor::VehiclePreview::EnsureForBlock(b);
            } else if (!shouldReplaceTarget && Editor::VehiclePreview::HasSpawn(placingBlockModel) && b !is null) {
                auto spawnLocal = Editor::VehiclePreview::SpawnLocalMat(placingBlockModel);
                Editor::VehiclePreview::EnsureAt(Editor::GetBlockMatrix(b) * spawnLocal, spawnLocal);
            } else if (placingItemModel !is null && Editor::VehiclePreview::HasSpawn(placingItemModel)) {
                auto spawnLocal = Editor::VehiclePreview::SpawnLocalMat(placingItemModel);
                mat4 host = itemMat;
                if (b !is null) host = Editor::GetBlockMatrix(b);
                Editor::VehiclePreview::EnsureAt(host * spawnLocal, spawnLocal);
            }
        }
