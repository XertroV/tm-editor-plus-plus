...
> **RESUME STATE (2026-08-18):** Phases 1–3 IMPLEMENTED and merged to master (commits `1c62997` devstructs, `2fae661` terrain support, `f4e99fc` reload-bug note). Implemented: TerrainSpec (shared) + TRNs serialization, map-grid capture with default-cell detection, native-MB terrain read via mb+0x1F8, donor fake-nod write (mb+0x1F8 + variant mirror, Force/offset1/frontiers), terrain-only ground-mode donor pass after the air-mode pass (delayed restore for the async apply), recorder `Record Terrain` toggle + capture-on-finish + conditional DoNotPlace, Current MB tab Terrains tree + variant info, recorder terrain counters.
> **PENDING: live verification** — blocked on a game restart (staged build has terrain; running game has pre-terrain E++). Verify: DevStruct reads vs pack dumps (`InspectMacroblockModel`, `GetMapTerrainGrid`), then record a terrain region and place it, diffing the grid. Untested risks: fake-nod layout vs the real apply path, ground-mode donor placement (first ever), immediate-vs-delayed restore timing.
> Also documented in AGENTS.md: the shared-type hot-reload limitation is structural (Openplanet builds the new module while the old one is registered; AS rejects changed shared declarations; restart required after any shared-type edit).
