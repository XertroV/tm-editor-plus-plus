...
> **STATUS (2026-08-18): COMPLETE and live-verified.** E++ donor-path terrain placement is byte-identical to native: 400/400 cell genealogy signatures match native placement at the same coord (87 terraformed cells, 313 flat). Key fixes found during live verification: (1) map grid offset must be hardcoded 0x390 (BlockStock anchor lands on 0x388); (2) zone nod resolution must use the map genealogy grid's typed `Zones` (CompleteZoneList names don't resolve); (3) the air pass must NOT carry terrain (air-mode + AutoTerrains = crash, the 01:42 crash); (4) fake genealogy `CurrentZone` = top zone of the stack (real entry: zones[1] with CurrentIndex=0), and tail 0x68..0x78 must be zeroed (real entries have zeros; map-cell templates don't); (5) never ground-place an empty donor (the 01:07 crash). Bisect experiment that isolated fake nods vs donor flow: aliasing the donor's mb+0x1F8 onto the real model's entries terraformed 114 cells natively → donor flow OK, fake nods were the problem (pack tool `AliasMacroblockAutoTerrains`).
