...
> **STATUS (2026-08-18): IMPLEMENTED; terraform verified working; final byte-fidelity check PARTIALLY done.** E++ donor-path terraforming works (87 cells from `PlaceMacroblockModelViaEppSpec` on `terrain-example`, 199/199 terrains captured from the native model). Fidelity vs native: 392/400 cells matched in a same-map comparison, but the reference dump was polluted (double placement + undersized 20x20 windows — the terrain-example pattern is **15 wide × 24 tall**, x 0..14 / z 0..23, not 20x20). The 8 delta cells sit at the formation frontier (6 raised transition cells, 2 dried cells) — unresolved whether E++ artifact or position/pollution artifact. A definitive clean A/B (E++ vs native on adjacent flat 16×24 areas (4,40) and (24,40), no plugin reloads between placement and dump — **plugin reloads kill the pending async terrain apply**) was repeatedly interrupted by game crashes (5+ this session, mostly unrelated to terrain work; two were terrain-path crashes, both fixed: empty-donor ground place at 01:07, terrain-in-air-pass at 01:42). Also note: the test map file on disk was saved WITH baked-in terrain at 01:54 — reloads no longer yield a flat map. `OpenMapInEditor` silently no-ops when a dialog blocks the map switch (filed https://github.com/clankercode/tm-control-mcp/issues/13). Key fixes verified live: hardcoded map grid 0x390; zone resolution via map grid (CompleteZoneList names don't resolve); air pass must not carry terrain; fake genealogy CurrentZone = top zone + zeroed tail; empty-donor guard. Bisect tool: `AliasMacroblockAutoTerrains` (proved donor flow works with real nods; fake nods were the only blocker).
