...
| E24 | MCP actual gizmo apply endpoint | Export E++ `Editor::Dev_RunGizmoApplyBlock` and add `tm-control-mcp` `RunGizmoApplyBlock` so Codex can call the actual gizmo apply path without marker files or manual UI gating | The exact sentinel-variant crash shape places through `_GizmoOnApply_Params(false)` with variant repaired and no crash, including repeated placement of the same macroblock/gizmo block | Passed live MCP gizmo tests and cleaned up | 2026-04-20 02:05: `RunGizmoApplyBlock` placed `RoadTechToThemeSnowRoad` at `<1080,190,620>` with requested variant `4294967295` and default autofocus. The actual gizmo apply path returned `placed=true`; readback showed freeblock coord `<4294967295,0,4294967295>` and repaired variant `0`. `RemoveRecentBlocks {"count":1}` then deleted the diagnostic block, returning the map to `2318` blocks / `2` items. A follow-up repeated smoke placed the same `RoadTechToThemeSnowRoad` sentinel variant twice in sequence at `<1110,190,620>` and `<1140,190,620>`; both returned `placed=true`, both read back repaired variant `0`, and `RemoveRecentBlocks {"count":2}` returned the map to `2318` blocks / `2` items. No matching recent `Editor` or `tm-control-mcp` exceptions were found after either smoke. | Manual UI risk is now narrowed to cursor/UI selection/input state; the actual gizmo apply function itself can be regression-tested through MCP |
| E25 | Gizmo setup cursor variant repair | Repair picked-block variants during `Gizmo_Setup` before calling `SetSelectedBlockInfo` / `SetCurrentBlockVariant` | The "nothing selected on cursor" symptom should not be caused by a stale sentinel variant reaching cursor setup before apply-time repair | Patched and apply-regressed live | 2026-04-20 02:xx: code inspection found the LMB/replace setup path copied `blockSpec.variant` into `targetVariant` before `EnsureValidVariant()`, so cursor variant setup could still see `4294967295` even though apply-time repair was fixed. `Gizmo_Setup` now calls `EnsureValidVariant()` immediately after constructing the picked block spec and aborts if no valid variant exists; `Dev_RunApplyBlock` also returns false if repair fails. E++ rebuilt/reloaded cleanly, `tm-control-mcp` reloaded as an Editor dependent, and a follow-up `RunGizmoApplyBlock` sentinel smoke placed and cleaned one `RoadTechToThemeSnowRoad` block, returning the map to `2318` blocks / `2` items with no matching recent `Editor` or `tm-control-mcp` exceptions. | Manual UI retry is still useful because synthetic apply cannot create a real picked block with corrupt stored variant after the constructor repair; if UI selection is still empty, instrument picker state rather than placement |
...
| E26 | Gizmo item replace delete crash | After buffer `AnchoredObjects.RemoveRange` + `UpdateNewlyAddedItems` “visual sync”, apply `PlaceItems` crashed native AV | Item gizmo replace must not crash; prefer engine delete only | Fixed in code (`63dac57`); awaiting live retest after TM restart | 2026-08-11 21:41:59 AEST crash `LogCrash_0000000000B9D61D` (archived `research/archive/LogCrash_2026-08-11_B9D61D_gizmo-item-delete.txt`). Openplanet.log: `UpdateNewlyAddedItems placed/removed true` then `wrote mb spec ... 0/1/0` (GreenCoast donor) then AV read `null+0xF0` (`rcx=0`). Root cause: buffer-remove leaves scene refs (UAF); `UpdateNewlyAddedItems` is for noticing *new* items, not healing deletes, and desynced the map. Earlier 21:28 path already used `RemoveRange` when engine delete no-op’d (count OK, visual delayed until apply). Fix matches `NudgeItemBlock`: `MwAddRef` + `DeleteBlocksAndItems(liveSpec)` only; no buffer remove, no post-delete `UpdateNewlyAddedItems`, no blind `PickedObject` free. Writeup: `research/archive/2026-08-11-GizmoItemDeleteCrash.md`. | Live-retest item gizmo after TM restart; if engine delete still no-ops, investigate item `RemoveMacroblock` match on non-Stadium donors rather than buffer fallback |
| E27 | Gizmo/magnet item scene phantoms + block cancel crash | Classify A/B/C ghosts; harmful Hide/len=0; cancel Undo+mode bounce | Safe ghost classing; no native crash on block gizmo cancel | Documented; harmful paths reverted; cancel fix in code | 2026-04-22: class C phantom after magnet+gizmo; Hide+len=0 caused Ctrl-hover ~16m trails; matrix-as-mobil crashed OP. Block gizmo cancel: Undo then SoftPlacementBounce during restore killed TM. Writeup: `research/archive/2026-04-22-GizmoItemScenePhantoms.md`. | Class C still needs safe hide RE. Cancel: deactivate first, defer Undo, bounce only for item targets. |
| E28 | Camera anim no-op vs double-tick freeze | Speculative RenderEarly+AfterMainLoop double-tick of `UpdateAnimAndCamera` during autofocus place+freeblock delete | Restore FocusCamera without freeze | **Harmful dead-end — reverted, not shipped** | 2026-08-12: anim "no-op" was shadow-compute modal (BasicDialogs often still `none`). After dismiss, anim worked without code change. Double-tick patch + autofocus cleanup froze TM (MCP dead, log stop at freeblock delete ~08:32:21). Writeup: `research/archive/2026-08-12-CameraAnimDoubleTickFreeze.md`. | Do not reintroduce double-tick. Future camera-tick fixes must pass autofocus+delete soak; detect/dismiss modals before blaming anim loop. |
| T3 | CanPlace patch + terrain apply engine analysis | Explain via Ghidra why partial applies happen and whether MacroblockCanPlacePatch is harmful | Know exactly what the patch masks and how the apply decides per-cell | **Passed — mechanism pinned; patch correlation likely confounded** | 2026-08-18: patch site = `CGameCtnEditorCommon::CanPlaceMacroBlock` @ 0x141164E90, JZ at +0x14116503D after the final validator call (vtable+0x268 = `CanPlaceTerrainFrontierBlocks`). Validator runs iff variant family count != 0 OR ground-variant AutoTerrains != 0 — so terrain-only macroblocks (0 blocks) still hit it; the patch forces return=true when it fails. `PlaceMacroBlock` @ 0x141166180: terrain section gated by `mb+0x12C`, `HasTerrainContent`, `CheckAutoTerrainPlaceType(variant,1,0)`; calls `ApplyAutoTerrains_GroundVariant` with **flags=1**, which disables the per-cell check `ValidateAutoTerrainCell` (0x140F40500 early-outs true when flags != 0) — native apply does NOT skip individual cells. Apply = build target genealogies per entry, then `ApplyTargetGenealogiesToGrid` = async BuildTerrain job: any `BuildTerrainJob_AddTargetGenealogy` setting the abort out-flag kills the WHOLE job (returns 0 → placement aborts, placed=false). So engine-side apply is all-or-nothing per job; observed partial terraforms = async job interrupted mid-rebuild (E++ reloads from the concurrent session, undo, possibly new placements superseding), not engine per-cell rejection. Live A/B same day: patch ON + cursed region → 0–21 cells; patch OFF + clean region + no reloads/undos during apply → 92/99/144 cells (E++ donor included). The patch on/off difference is confounded with region history and reload timing — patch is NOT proven to break applies, but it does mask genuine validator rejections (canPlace=true on bad spots). Renamed 10 functions incl ProfileScope_Enter/Exit, BuildTerrainJob_*; plate comment on CanPlaceMacroBlock; project saved. | Re-run the clean fidelity A/B with NO E++ reloads/undos during applies (coordinate with the concurrent agent); consider narrowing the patch to skip only non-terrain macroblocks, or have E++ warn when patched canPlace is relied on for terrain placements |
| T2 | E++ terrain placement (donor path) | Implement TerrainSpec + capture + fake-nod donor write + ground-mode terrain pass; verify vs native | Byte-identical terraform to native placement | **Mostly passed — terraform works; fidelity 392/400 pending final A/B** | 2026-08-18: `PlaceMacroblockModelViaEppSpec` on `terrain-example` terraformed 87 cells via donor+fake nods (spec captures 199/199 terrains from native model). Fixes found live: hardcode map grid 0x390 (BlockStock anchor → 0x388); zone resolution via map grid not CompleteZoneList; air pass must not carry terrain (crash); fake genealogy CurrentZone = top zone, tail 0x68..0x78 zeroed; never ground-place empty donor (crash). Bisect: aliasing donor mb+0x1F8 to real entries terraformed 114 cells → fake nods were the only blocker. Fidelity vs native control: 392/400 cells match; 8 frontier-cell delta unresolved (comparison polluted by double placement + undersized windows; pattern is 15×24 not 20×20). Clean A/B on adjacent flat areas prepared (E++ dx/dz=4,40 vs native (24,14,40)) but interrupted by repeated game crashes. NOTE: plugin reloads kill the pending async terrain apply. | Finish clean A/B fidelity check; terrain follows spec min XZ only (moving specs with terrain is future work); removal of placed terrain not wired; test map file on disk now has baked-in terrain (saved 01:54) — recreate a flat one for future tests |
| T4 | Clean RedIsland A/B after EditNewMap | Fresh 64³ RedIsland Day via EditNewMap (flat sig (15,14), 4096 blocks); patch OFF; native PlaceMacroblock (8,14,8) vs E++ donor PlaceMacroblockModelViaEppSpec dx/dz=28 | Native and E++ both terraform the same cells on a never-saved flat map | **Partial — native fragments, E++ 0 cells** | 2026-08-18 evening: `EditNewMap RedIsland` opened Unnamed 64³. `terrain-example` Inspect = 199 AutoTerrains. Patch `applied:false`. Native `canPlace=true placed=true` → **109 cells** changed in 0,0 32×32 (sigs (15,17)=46, (15,23)=20, (15,19)=20, (15,17,19)=12, (15,19,23)=2, (15,31)=9; bbox x9–30 z9–21). E++ `placed=true nbTerrains=199` at +28,+28 → **0 cells** in 28,28 32×32 (all still (15,14)). Reproduces the scripted-native-fragments vs E++-zero split on a clean map with no reloads/undos during apply. DumpMacroblockAutoTerrainRaw index 0 now reads the 199-entry model (prior len=1 was a resolve miss). | Next: compare E++ donor write (fake nods / ground pass) against this zero-apply; native 109/199 is still short of the manual-UI ~94-cell full motif (different spot) — reconstruct expected offsets from all 199 entries (span may be ~22×13 here, not 15×24) |
| T1 | Macroblock terrain mechanism | Identify in-memory terrain representation and native apply path using `RedIsland\terrain-example.Macroblock.Gbx` on `redisland-mb-test` | Know exactly what to write for E++ terrain support | **Passed — mechanism pinned** | 2026-08-17: terrain = `CGameCtnMacroBlockInfo+0x1F8` buffer of `CGameCtnAutoTerrain` (offset + `CGameCtnZoneGenealogy`); variant copy (+0x250) NOT used for place (used by remove/ground blocks). Zeroing mb len kills terraforming, zeroing variant len does not. Map terrain = genealogy per XZ cell at `CGameCtnChallenge+0x390`. Apply is async (~1s), heights shift by placementY+1, zone nods shared per collection. New pack tools: `GetMapTerrainGrid`, `PlaceMacroblockModelNative`, `SetMacroblockAutoTerrainLen`, AutoTerrains in `InspectMacroblockModel`. ~35 Ghidra functions renamed, 3 structs, plate comments (project saved). Full writeup: `research/MacroblockTerrain.md`. | Implement per plan in `research/MacroblockTerrain.md`: spec TerrainSpec, capture from map grid, write mb+0x1F8 (+variant mirror) into donor, ground-mode place; test ground-mode donor placement |
## Context
