...
| T2 | E++ terrain placement (donor path) | Implement TerrainSpec + capture + fake-nod donor write + ground-mode terrain pass; verify vs native | Byte-identical terraform to native placement | **Passed — 400/400 cells match native** | 2026-08-18: `PlaceMacroblockModelViaEppSpec` on `terrain-example` at (0,14,0) terraformed 87 cells; grid dump matches native `PlaceMacroblockModelNative` at same coord exactly (400/400 genealogy signatures). Fixes found live: hardcode map grid 0x390 (BlockStock anchor → 0x388); zone resolution via map grid not CompleteZoneList; air pass must not carry terrain (crash); fake genealogy CurrentZone = top zone, tail 0x68..0x78 zeroed; never ground-place empty donor (crash). Bisect: aliasing donor mb+0x1F8 to real entries terraformed 114 cells → fake nods were the only blocker. | Terrain follows spec min XZ only (moving specs with terrain is future work); removal of placed terrain not wired |
